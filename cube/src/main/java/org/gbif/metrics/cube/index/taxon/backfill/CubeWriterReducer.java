...
public class CubeWriterReducer extends Reducer<Key, IntWritable, NullWritable, NullWritable> {
  ...
  protected void reduce(Key key, Iterable<IntWritable> values, Context context) throws IOException {
    ...
    context.setStatus("Dataset[" + key.getDatasetKey() + "],  NubKey[" + key.getNubKey() + "], Count[" + total + "]");
    try {
      UuidIntMap c = UuidIntMap.newInstance(ImmutableMap.<UUID, Integer>of(key.getDatasetKey(), total));
      WriteBuilder wb =
        new WriteBuilder(OccurrenceDatasetCube.INSTANCE).at(OccurrenceDatasetCube.NUB_KEY, key.getNubKey());
  ...
  protected void setup(Context context) throws IOException, InterruptedException {
    ...
    hbaseCube =
      HBaseCubes.newCombiningBatchAsync(OccurrenceDatasetCube.INSTANCE, UuidIntMap.DESERIALIZER,
